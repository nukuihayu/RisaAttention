...
// Threads cooperate along N with vectorized 128-bit loads, warp-shuffle range
// reduction, and reverse Pass 2 iteration for L2 cache reuse. Pass 1 uses 4x
...
__device__ __forceinline__ void load_tile(const T *ptr, float *out_vals) {
  if constexpr (std::is_same_v<T, float>) {
    ...
    const T *loaded = risa::load_f16x8(ptr);
#pragma unroll
...
quant_v_int8_kernel(const T *__restrict__ v, int8_t *__restrict__ out,
                   ...
                   float *__restrict__ center_out, int N, int padded_N, int H,
  ...
  // Pass 1: per-channel range. Midpoint centering minimizes the worst-case
  // symmetric INT8 step for every channel.
  float mn[kDTile];
  float mx[kDTile];
#pragma unroll
  for (int i = 0; i < kDTile; ++i) {
  ...
  // 4x unrolled: issue 4 independent 128-bit loads per iteration
...
#pragma unroll
    for (int di = 0; di < kDTile; ++di) {
      mn[di] = fminf(mn[di], fminf(fminf(t0[di], t1[di]), fminf(t2[di], t3[di])));
      mx[di] = fmaxf(mx[di], fmaxf(fmaxf(t0[di], t1[di]), fmaxf(t2[di], t3[di])));
    }
  }
  for (; n < N; n += THREADS) {
    float tmp[kDTile];
...
#pragma unroll
    for (int di = 0; di < kDTile; ++di) {
      mn[di] = fminf(mn[di], tmp[di]);
      mx[di] = fmaxf(mx[di], tmp[di]);
    }
  }
...
#pragma unroll
  for (int di = 0; di < kDTile; ++di) {
    mn[di] = -risa::warp_reduce_fmax(-mn[di]);
    mx[di] = risa::warp_reduce_fmax(mx[di]);
  }

  // Cross-warp reduction
  __shared__ float warp_mn[kDTile][WARPS];
  __shared__ float warp_mx[kDTile][WARPS];
  __shared__ float inv_sc_sh[kDTile];
  __shared__ float center_sh[kDTile];
  __shared__ float warp_residual_sum[kDTile][WARPS];

  if (lane == 0) {
#pragma unroll
    for (int di = 0; di < kDTile; ++di) {
      warp_mn[di][warp] = mn[di];
...
#pragma unroll
    for (int w = 0; w < WARPS; ++w) {
      min_val = fminf(min_val, warp_mn[threadIdx.x][w]);
      max_val = fmaxf(max_val, warp_mx[threadIdx.x][w]);
    }

    const float center = 0.5f * (min_val + max_val);
    const float radius = 0.5f * (max_val - min_val);
    const float sc = fmaxf(radius * (1.f / 127.f), 1e-12f);
    const int parameter_index = (b * H + h) * D + d0 + threadIdx.x;
    scale_out[parameter_index] = sc;
    inv_sc_sh[threadIdx.x] = 1.f / sc;
    center_sh[threadIdx.x] = center;
  }
  __syncthreads();
...
#pragma unroll
  for (int di = 0; di < kDTile; ++di) {
    ...
    center[di] = center_sh[di];
  }

  // Pass 2: quantize and permute in reverse order for L2 reuse.
...
#pragma unroll
    for (int di = 0; di < kDTile; ++di) {
      const float normalized = (tmp[di] - center[di]) * inv_sc[di];
      const int8_t code = risa::float_to_int8_rn(normalized);
      out[(out_row + di) * padded_N + dst] = code;
      residual_sum[di] += normalized - static_cast<float>(code);
    }
  }

#pragma unroll
  for (int di = 0; di < kDTile; ++di) {
    residual_sum[di] = warp_reduce_sum(residual_sum[di]);
  }
  if (lane == 0) {
#pragma unroll
    for (int di = 0; di < kDTile; ++di) {
      warp_residual_sum[di][warp] = residual_sum[di];
    }
  }
  __syncthreads();

  // For the fixed midpoint codes and scale, add the mean normalized rounding
  // residual. This is the least-squares-optimal reconstruction center and
  // removes the per-channel DC residual without separately reducing sum(V).
  if (threadIdx.x < kDTile) {
    float total_residual = 0.0f;
#pragma unroll
    for (int w = 0; w < WARPS; ++w) {
      total_residual += warp_residual_sum[threadIdx.x][w];
    }
    const float scale = 1.0f / inv_sc_sh[threadIdx.x];
    const int parameter_index = (b * H + h) * D + d0 + threadIdx.x;
    center_out[parameter_index] =
        center_sh[threadIdx.x] +
        scale * (total_residual / static_cast<float>(N));
  }
...
extern "C" void launch_quant_v_int8_kernel(const void *v, void *out, void *scale,
                                          void *center, int B, int H, int N, int D,
  ...
  DISPATCH_FP_DTYPE(input_dtype_code, T, [&] {
    if (N <= 256) {
      quant_v_int8_kernel<T, 128><<<blocks, 128, 0, stream>>>(
          ...
          static_cast<float *>(scale), static_cast<float *>(center), N,
          padded_N, H, D, sb, sh, sn);
    } else {
